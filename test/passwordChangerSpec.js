...
describe('passwordChanger', function () {
  it('form based auth: should successfully change password for user1', function (done) {
    var requestBody = {
  ...
  it('basic auth: should successfully change password for user2', function (done) {
    var requestBody = {
      newPassword: 'changed2'
    }
    request.put(urljoin(opts.profileManagementServiceGatewayUrl, '/users/' + testdata.users.user2.logins.local.login + '/password'))
      .auth(testdata.users.user2.logins.local.login, testdata.users.user2.password)
      .set('Content-Type', 'application/json')
      .set('Accept', 'application/json')
      .send(JSON.stringify(requestBody))
      .end(function (err, res) {
        ...
        function requestUserDocFromCouchDb () {
          ...
          client.basicAuth(testdata.users.user2.name, testdata.users.user2.password)
          client.get('/_users/' + testdata.users.user2._id, function (err, req, res, obj) {
            if (err && err.statusCode === 401) {
              // this should happen, as we used the OLD password for requesting the users document
              done()
              return
  ...
  it('should fail for unknown user (404 - NOT FOUND)', function (done) {
    var requestBody = {
    ...
    request.put(urljoin(opts.profileManagementServiceGatewayUrl, '/users/unknownLocalLogin/password'))
      ...
      .end(function (err, res) {
        if (err && res.body.message.id === 'USER_NOT_FOUND') {
          ...
          assert.equal(res.statusCode, 404, 'StatusCode expected to be 404 - NOT FOUND')
          done()
          return
        }
